...
`0.3.7` is already on `origin/main` (`bb19381`) and is **not** on General. Register
that commit first (`@JuliaRegistrator register` on [issue #3](https://github.com/SimonAB/CausalTargeted.jl/issues/3)
while `Project.toml` still says `0.3.7`). Then push `0.3.8` and register it.
Do not skip 0.3.7: Mediation `[sources]` and dependents expect the schema export
as a registered 0.3.7+ version.
